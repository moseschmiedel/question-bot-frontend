...
import FeatherIcons
import Colors.Opaque exposing (black, white, red)
import Ui exposing (..)
...
type User = Username String | Anonymous
type QuestionId = QuestionId Int

type Votes = Voted Int | Notvoted Int
...
type Question = OpenQuestion OpenQuestionRecord | AnsweredQuestion AnsweredQuestionRecord
...
fOnSingleQuestion questionId f question =
  case question of
    OpenQuestion q -> if questionId == q.id then f question else question
    AnsweredQuestion q -> if questionId == q.id then f question else question
...
increaseQuestionId (QuestionId id) = QuestionId <| id + 1
